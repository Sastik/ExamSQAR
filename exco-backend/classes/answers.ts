...
export class AnswerOps {
  static async create(
    ...
    try {
      ...
      if (!res.acknowledged)
        return Result.failure<Question[]>(
      ...
      if (examObj.hasScripts !== true) {
        ...
        if (!examUpdate.acknowledged)
          return Result.failure<Question[]>("Failed to update exam").setExtra(
            500
          );
      }
  ...
  static async getSubmission(
    ...
    try {
      const submission = await DBCon.colls[COL_ANS].findOne({
        exam: examID,
        givenBy: studentID,
      });
      if (submission === null)
        return Result.failure<Submission>("Submission not found").setExtra(404);
      return Result.success<Submission>(submission);
    } catch (e) {
      console.error(e);
      return Result.failure<Submission>("Failed to get submission").setExtra(
        500
      );
    }
  }

  static async getOwnedSubmissions(
    studentID: ObjectId,
    newest: boolean,
    limit: number,
    cursor: number
  ): Promise<Result<MicroExamInfo[]>> {
    try {
      const ag = await DBCon.colls[COL_QUES].aggregate([
        {
          $lookup: {
            from: DBCon.collNames[COL_ANS],
            let: { examId: "$_id" },
            pipeline: [
              {
                $match: {
        ...
        { $unwind: { path: "$submissions", preserveNullAndEmptyArrays: true } },
        {
          $project: {
            ...
            questions: 1,
            hasScripts: 1,
            subID: "$submissions._id",
            subStartTime: "$submissions.startTime",
            subFinished: "$submissions.finished",
          },
        },
        { $sort: { subStartTime: newest ? -1 : 1 } },
        { $skip: cursor },
        { $limit: limit },
      ]).toArray();
      ...
      const res = ag.map(
        (s): MicroExamInfo => ({
          ...
          subStartTime: s.subStartTime,
          subFinished: s.subFinished,
        })
      );
      ...
      console.error(e);
      return Result.failure<MicroExamInfo[]>(
  ...
  static async getState(
    ...
    studentID: ObjectId,
    fillableExam?: Exam
  ): Promise<Result<SubmitState>> {
    const extras =
      fillableExam !== undefined ? { title: 1, description: 1, madeBy: 1 } : {};
    try {
      ...
      if (_exam === null)
        return Result.success<SubmitState>(SubmitState.EXAM_NOT_FOUND).setExtra(
          404
        );
      ...
      if (
        ...
        return Result.success<SubmitState>(SubmitState.EXAM_ERROR).setExtra(
          500
        );
      ...
      const _sub = await DBCon.colls[COL_ANS].findOne(
        { exam: examID, givenBy: studentID },
        { projection: { finished: 1, startTime: 1 } }
      );
      ...
      if (hasSubmission) {
        ...
        const canCarryOn = AnswerOps.hasTime(
          windowEnd,
          startTime,
          duration,
          clampTime
      ...
      console.error(e);
      return Result.failure<SubmitState>(
        "Failed to check submission state"
      ).setExtra(500);
    }
  }

  static async removeAllUnderExam(
    examID: ObjectId,
    mark: boolean
    ...
    try {
      ...
      if (mark) {
        await DBCon.colls[COL_QUES].updateOne(
          { _id: examID },
          { $set: { hasScripts: false } }
        );
      }
      return Result.success<number>(res.deletedCount);
    } catch (e) {
      console.error(e);
      return Result.failure<number>("Failed to delete submissions").setExtra(
        500
      );
    }
  }

  static async removeStudentSubmission(
    examID: ObjectId,
    studentID: ObjectId
  ): Promise<Result<void>> {
    try {
      const res = await DBCon.colls[COL_ANS].deleteOne({
        exam: examID,
        givenBy: studentID,
      });
  ...
  static async markAsFinished(
    ...
    try {
      ...
      return Result.failure<void>(
        "Failed to mark submission as finished"
      ).setExtra(500);
    }
  }

  static async getSubmissionsWithUser(
    examID: ObjectId
  ): Promise<Result<SubmissionWithUser[]>> {
    try {
      ...
      return Result.success<SubmissionWithUser[]>(
        submissions as SubmissionWithUser[]
      );
    } catch (e) {
      console.error(e);
      return Result.failure<SubmissionWithUser[]>(
        "Failed to get submissions"
      ).setExtra(500);
    }
  }
  ...
  static async attemptQuestion(
    ...
    try {
      ...
      if (exam.windowStart && Date.now() < exam.windowStart.getTime()) {
        return Result.failure<void>(
          "Cannot attempt outside of exam window"
        ).setExtra(406);
      }
      ...
      if (sub.finished)
        return Result.failure<void>("Exam has already been finished").setExtra(
          406
        );

      if (sub.answers === undefined || sub.answers?.length === 0)
        return Result.failure<void>("Question submission not found").setExtra(
          404
        );
      ...
      if (
        ...
        return Result.failure<void>(
          "Cannot submit outside of exam duration"
        ).setExtra(406);
      }
      ...
      const optionLength = (exam.questions[0] as { optionLength: number })
        .optionLength;

      if (
        ans.usedAttempts !== undefined &&
      ...
      if (update.modifiedCount === 0)
        return Result.failure<void>(
          "Failed to proceed into submission"
        ).setExtra(500);

...
export class UserOps {
  static async create(user: User): Promise<Result<boolean>> {
    try {
      const existingUser = await DBCon.colls[COL_USER].findOne({
        email: user.email,
      });
      if (existingUser) {
        return Result.failure<boolean>("Email already exists").setExtra(400);
      }
  ...
  static async authorize(
    ...
    try {
      ...
      console.error(e);
      return Result.failure<User | null>("Failed to authorize user").setExtra(
        500
      );
    }
  }
  ...
  static async update(user: User): Promise<Result<void>> {
    ...
    try {
      const res = await DBCon.colls[COL_USER].updateOne(
        { _id },
        { $set: update }
      );
  ...
  static async getInfo(id: ObjectId): Promise<Result<User>> {
    try {
      ...
      const user = await DBCon.colls[COL_USER].findOne(
        { _id: id },
        { projection: { password: 0 } }
      );
      if (user === null) {
        return Result.failure<User>(`No user exists with id ${id}`).setExtra(
          404
        );
      } else {

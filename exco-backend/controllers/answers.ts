...
function substateToDlvState(state: SubmitState | null): ExamDeliveryState {
  if (state === null) return ExamDeliveryState.GEN_EXAM_ERROR;

  switch (state) {
    case SubmitState.EXAM_NOT_STARTED:
      return ExamDeliveryState.STD_EXAM_NOT_STARTED;
    case SubmitState.EXAM_STARTED:
      return ExamDeliveryState.STD_EXAM_STARTED;
    case SubmitState.EXAM_SUBMITTED:
      return ExamDeliveryState.STD_EXAM_SUBMITTED;
    case SubmitState.EXAM_DONE_EXPIRED:
      return ExamDeliveryState.STD_EXAM_DONE_EXPIRED;
    case SubmitState.EXAM_MISSED:
      return ExamDeliveryState.STD_EXAM_MISSED;
    case SubmitState.EXAM_EARLY:
      return ExamDeliveryState.STD_EXAM_EARLY;
    case SubmitState.EXAM_NOT_FOUND:
      return ExamDeliveryState.STD_EXAM_NOT_FOUND;
    case SubmitState.EXAM_ERROR:
...
function showQues(state: ExamDeliveryState, hasScores: boolean): boolean {
  return (
    ...
    (hasScores &&
      (state === ExamDeliveryState.STD_EXAM_DONE_EXPIRED ||
...
function validateQAList(
  ques: Question[] | null,
  ans: Answer[] | null
): boolean {
  if (ques === null || ans === null) return true;

  if (ques !== null && ans === null) return true;

  if (ques === null && ans !== null) return false;
  ...
  if (Array.isArray(ques) && Array.isArray(ans)) {
    ...
    for (let i = 0; i < ques.length; i++) {
      ...
      if (q._id === undefined || a.question === undefined) return false;

      if (!q._id.equals(a.question)) return false;
    }
...
export class AnswerService {
  ...
  async createScript(
    ...
    @urlParam("id", true, { convertTo: Str, validate: IsMongoID })
    examID: string
  ) {
    if (role !== UserRole.STUDENT) {
      return Result.failure(
        "Your role does not allow you to start exams"
      ).setExtra(403);
    }
    ...
    if (state === SubmitState.EXAM_NOT_STARTED) {
      ...
      return Result.failure(
        `You cannot start this exam. Reason: ${
          state ? SubmitState[state] : "Unknown error"
        }`
      ).setExtra(406);
    }
  }
  ...
  async attemptQuestion(
    ...
    @urlParam("id", true, { convertTo: Str, validate: IsMongoID })
    examID: string,
    @urlParam("qid", true, { convertTo: Str, validate: IsMongoID })
    questionID: string,
    @bodyValue("provided", true, { validate: IsNumberArray }) provided: number[]
  ) {
    if (role !== UserRole.STUDENT) {
      return Result.failure(
        "Your role does not allow you to attempt questions"
      ).setExtra(403);
    }
  ...
  async finishExam(
    ...
    if (role !== UserRole.STUDENT) {
      return Result.failure(
        "Your role does not allow you to finish exams"
      ).setExtra(403);
    }
  ...
  async getExam(
    ...
    @urlParam("id", true, { convertTo: Str, validate: IsMongoID })
    mainID: string,
    @queryParam("std", false, { convertTo: Str, validate: IsMongoID })
    stdID: string
  ) {
    ...
    if (role === UserRole.STUDENT) {
      const examOBJ: Exam = {};
      const _state = await AnswerOps.getState(
        new ObjectId(mainID),
        new ObjectId(userID),
        examOBJ
      );
      ...
      if (showQues(dlvState, showScores)) {
        ...
        const _sub = await AnswerOps.getSubmission(
          examID,
          new ObjectId(userID)
        );
        ...
        if (
          showCorrects(dlvState, showScores) &&
          questions !== null &&
          subAnswers !== null
        ) {
          const _res = scorify(
            questions as Qstruct[],
            subAnswers as Astruct[],
            subID
          );
          if (!_res.ok)
            return Result.failure("Error processing grades").setExtra(500);
  ...
  async getSubmissions(
    ...
    if (role !== UserRole.TEACHER) {
      return Result.failure(
        "Your role does not allow you to view submissions"
      ).setExtra(403);
    }

    const _ownerCheck = await ExamOps.checkIfOwner(
      new ObjectId(examID),
      new ObjectId(userID)
    );
  ...
  async getExams(
    @jwtValue("id") userID: string,
    @jwtValue("role") role: number,
    @queryParam("newest", false, { convertTo: Bool, validate: IsBoolStr })
    newest?: boolean,
    @queryParam("limit", false, { convertTo: Num, validate: IsIntStr })
    limit?: number,
    @queryParam("skip", false, { convertTo: Num, validate: IsIntStr })
    skip?: number
  ) {
    newest = newest || false;
    limit = limit || 20;
    skip = skip || 0;
    if (role === UserRole.STUDENT) {
      const res = await AnswerOps.getOwnedSubmissions(
        new ObjectId(userID),
        newest,
        limit,
        skip
      ...
      const res = await ExamOps.getOwnedExams(
        new ObjectId(userID),
        newest,
        limit,
        skip

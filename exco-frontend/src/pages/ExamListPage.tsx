...
const ExamListPage: React.FC = () => {
  ...
  useEffect(() => {
    new ExcoFetcher()
      ...
      .get(`api/exam/`)
      .failure((msg) =>
        setLden({ state: LoadState.WARNING, exam: null, msg, role: null })
      )
      .error((msg) =>
        setLden({ state: LoadState.ERROR, exam: null, msg, role: null })
      )
      .success((data) => {
        if (
          typeof data !== "object" ||
        ...
        const { list, role } = data as {
          list: ExamMetaDataT[] | ExamMetaDataS[];
          role: UserRole;
        };
        ...
        switch (role) {
          ...
          case UserRole.STUDENT:
            setLden({
              state: LoadState.LOADED,
              exam: list as ExamMetaDataS[],
              role,
            });
            break;
          default:
            setLden({
              state: LoadState.WARNING,
              exam: null,
              msg: "Unknown Role Access",
              role: null,
            });
            break;
        }

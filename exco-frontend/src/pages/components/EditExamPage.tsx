...
const EditExamPage: React.FC<{ examData: ExamData }> = ({ examData }) => {
  ...
  function addNewQues() {
    const qs = new Question(null, "", 1, 1, QuesType.RADIO, [], []);
  ...
  function saveQues(
    ...
    if (isNew) {
      exf.post(`api/exam/${examData.examID}/ques`).success((res) => {
        messageApi.success("Question added successfully");
        settleQues(index, { ...quesData, _id: res }, res);
        setEditIndex(-1);
      });
    } else {
      exf.patch(`api/exam/${examData.examID}/ques/${quesID}`).success(() => {
  ...
  return (
    <>
      ...
      <Space
        direction="vertical"
        size="middle"
        style={{ display: "flex", margin: "20px 0px", width: "100%" }}
      >
        {questions.map((ques, i) =>
          editIndex === i ? (
            <EditQues
              ...
              onEditSave={(quesData, setSpinner) =>
                saveQues(i, ques._id, quesData, setSpinner)
              }
              limitedEdit={limitedEdit}
            />
          ) : (
            <ViewQues
              ...
              onDelete={(setSpinner) =>
                deleteQues(i, ques._id as string, setSpinner)
              }
              limitedEdit={limitedEdit}
            />
          )
        )}
        <div style={{ height: "48px", display: "block" }}>
          <AddQuestion

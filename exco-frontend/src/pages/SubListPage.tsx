  ...
  return (
    <>
      <div style={{ float: "right", margin: "10px 6px" }}>
        <Text type="secondary" style={{ margin: "0px 5px" }}>
          Generate Gradesheet
        </Text>
        <Select
          defaultValue={format}
          style={{ width: 120, margin: "0px 5px" }}
      ...
      <Table dataSource={subs} pagination={false} scroll={{ y: "70vh" }}>
        <ColumnGroup title="Name">
          <Column
            title="First Name"
            dataIndex={["givenBy", "firstName"]}
            key="givenBy.firstName"
          />
          <Column
            title="Last Name"
            dataIndex={["givenBy", "lastName"]}
            key="givenBy.lastName"
          />
        </ColumnGroup>
        <Column
          title="Start Time"
          ...
          key="startTime"
          render={(dateStr) =>
            new Date(dateStr).toLocaleString("en-GB").toLocaleUpperCase()
          }
        />
        <Column
          ...
          key="finished"
          render={(finished) =>
            finished ? <Tag color="green">Yes</Tag> : <Tag color="red">No</Tag>
          }
        />
        <Column
          title="View"
          dataIndex={["givenBy", "_id"]}
          key="givenBy._id"
          render={(userID) => (
            <Button href={`/exam/${examID}/by/${userID}`}>Open</Button>
          )}
        />
      </Table>
    </>
...
const SubListPage: React.FC = () => {
  ...
  const [lden, setLden] = useState<LoadEntity>({
  ...
  switch (lden.state) {
    ...
    case LoadState.WARNING:
      toDisplay = (
        <LoadError subTitle={lden.msg} status={errorStateToText(lden.state)} />
      );
      break;
  }
  ...
  return (
    <>
      <NavMenu
        pseudoRoute={{ title: "Submissions", key: "subs" }}
        selected="subs"
      />
